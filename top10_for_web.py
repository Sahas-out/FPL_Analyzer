...
df = pd.read_excel('Top-Forwds.xlsx') # edit name here
chart = pd.read_csv('./images/forwards_charts.csv')# edit name here
...
df['xfpl&f'] ="xfpl&f " + df['xfpl&f'].round(2).astype(str)   
df['price'] ="price  " + df['price'].round(2).astype(str)   
df['VS'] ="VS " + df['VS'].astype(str)   
df['charts'] = chart['url']

driver.quit()
df[['name','img-urls','xfpl','xfpl&f','VS','price','chance_playing','charts']].to_csv('./web_display/For.csv',index=False)